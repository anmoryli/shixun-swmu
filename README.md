...
- 管理员（`ROLE_1`）维护基础数据、医生账号、政策、药品、销售地点和必备材料，可执行写入、删除和医生密码重置。
- 医生/普通用户（`ROLE_2`）登录后读取授权菜单和业务数据；写入权限由后端方法级授权控制，不能只依赖前端按钮隐藏。
...
安全实现包括 BCrypt 密码校验、Spring Security 方法级角色授权、httpOnly Cookie 会话、Redis 会话摘要/滑动续期、上传 MIME 与真实图片校验、UUID 文件名和路径穿越防护。浏览器默认不读取 token；脚本客户端仍可按后端兼容规则使用 Authorization 头。
...
3. `GET /api/permissions` 根据 Redis 认证主体角色返回菜单，不信任客户端角色名。
4. `POST /api/logout` 删除当前会话并清理 Cookie。
5. 后端过滤器仍兼容 `Authorization: Bearer <token>` 或裸 token，方便 Postman/脚本；浏览器前端优先使用 Cookie。
...
| SQL 迁移语法错误 | 客户端和执行顺序 | V2/V3 需要 `DELIMITER`；显式指定 `medicine` |
