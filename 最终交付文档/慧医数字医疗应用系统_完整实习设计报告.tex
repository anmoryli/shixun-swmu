...
登录时后端校验账号状态和密码摘要，成功后生成随机令牌，将令牌摘要与会话信息写入 Redis，并通过 httpOnly Cookie 交付浏览器。后续请求由安全过滤链提取 Cookie，校验 Redis 会话并恢复真实角色。前端通过会话接口恢复登录态，通过权限接口生成动态路由；服务端仍对每个写接口执行最终授权，避免仅依赖前端隐藏按钮。
...
数据库使用 \texttt{medicine} 模式，以账号、医生、企业、城市、销售地点、药品、医保政策、公司政策和材料等表保存核心数据。基线脚本定义初始结构，迁移脚本逐步补充业务字段、索引、经纬度坐标和运行所需约束。当前迁移完成后共包含 20 张业务及关联表。
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{p{3.3cm}p{3.7cm}L}
    ...
    表或领域 & 关键字段 & 设计说明 \\
    \midrule
    账号与角色 & account、password、role、status & 保存认证主体、摘要密码、角色和启停状态 \\
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{p{3.2cm}p{3.0cm}L}
    ...
    V2 迁移 & 结构补齐 & 补充后端接口所需字段、约束和索引 \\
    V3 迁移 & 地图扩展 & 增加销售地点经纬度和相关查询能力 \\
    V4 迁移 & 运行完善 & 调整最新业务、测试和部署需要的数据结构 \\
    \bottomrule
  \end{tabularx}
\end{table}

初始化顺序为创建数据库、执行基线脚本、按版本顺序执行迁移、校验表数量与关键索引，最后启动应用。生产环境执行迁移前必须备份数据库，并保证脚本具备可追踪版本，不允许直接修改已经发布的历史迁移文件。
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{p{3.1cm}p{2.5cm}p{2.5cm}L}
    ...
    测试层次 & 通过数 & 总数 & 结论 \\
    \midrule
    后端 JUnit 测试 & 84 & 84 & 全部通过 \\
    前端 Vitest 测试 & 123 & 123 & 全部通过 \\
    Python 测试 & 8 & 8 & 全部通过 \\
    合计 & 215 & 215 & 通过率 100\% \\
    \bottomrule
  \end{tabularx}
...
正式子域名尚未在仓库资料中确认，因此本报告不虚构上线地址。部署前应将 DNS 记录指向 ECS 或负载均衡，配置 TLS 证书，并同步修改 Nginx \texttt{server\_name}、后端 CORS 白名单与 Cookie 安全策略。正式地址填写为：\placeholder{正式子域名}。
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{p{3.5cm}p{4.2cm}L}
    ...
    配置项 & 建议值 & 说明 \\
    \midrule
    DNS & \placeholder{正式子域名} 指向 ECS/负载均衡 & 根据公网网络填写 A、AAAA 或 CNAME \\
    Nginx \texttt{server\_name} & \placeholder{正式子域名} & 替换演示配置中的主机名 \\
    CORS & \texttt{https://}\placeholder{正式子域名} & Cookie 模式不能与通配符同时使用 \\
...
本项目完成了从需求分析、UML 建模、数据库设计、前后端实现、自动化测试到容器部署和持续集成的完整软件工程闭环。系统形成管理员与医生两类角色的访问边界，覆盖八类核心业务管理及首页、地图、图片和健康检查能力。代码级测试达到 215/215 通过，后端与前端行覆盖率达到 100\%，历史部署环境的 57 项 API 回归全部通过。
...
\begin{longtable}{p{2.1cm}p{4.8cm}p{2.4cm}p{4.2cm}}
  ...
  GET & \texttt{/api/permissions} & 已登录 & 按真实角色返回权限树 \\
  POST & \texttt{/api/logout} & 已登录 & 删除 Redis 会话并清除 Cookie \\
  GET & \texttt{/api/dashboard} & 管理员/医生 & 返回首页统计、分布和资讯 \\
  GET & \texttt{/actuator/health} & 网关策略控制 & 返回应用、MySQL 和 Redis 状态 \\
  GET & \texttt{/api/regeo} & 已登录 & 高德逆地理编码代理 \\
  \bottomrule
\end{longtable}
...
\begin{longtable}{p{2.5cm}p{5.2cm}p{5.8cm}}
  ...
  \midrule
  \endhead
  城市 & \texttt{GET /api/citys}、分页查询 & \texttt{POST /api/citys}、\texttt{DELETE /api/citys/\{id\}} \\
  公司 & \texttt{GET /api/companys}、分页查询 & \texttt{POST}、\texttt{PUT/DELETE /api/companys/\{id\}} \\
  销售地点 & \texttt{GET /api/sales}、分页查询 & \texttt{POST}、\texttt{PUT/DELETE /api/sales/\{id\}} \\
  公司政策 & \texttt{GET /api/company\_policys} & \texttt{POST}、\texttt{PUT/DELETE /api/company\_policys/\{id\}} \\
  医保政策 & \texttt{GET /api/medical\_policys} & \texttt{POST}、\texttt{PUT/DELETE /api/medical\_policys/\{id\}} \\
  医生 & \texttt{GET /api/doctors}、\texttt{/doctors/info} & \texttt{POST}、\texttt{PUT/DELETE}、密码重置 \\
  药品 & \texttt{GET /api/drugs}、分页查询 & \texttt{POST}、\texttt{PUT/DELETE /api/drugs/\{id\}} \\
  材料 & \texttt{GET /api/materials} & \texttt{POST}、\texttt{PUT/DELETE /api/materials/\{id\}} \\
  图片 & \texttt{GET /image/\{name\}} & \texttt{POST /api/base/upload} \\
  \bottomrule
\end{longtable}
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{p{1.4cm}p{4.5cm}L}
    ...
    2 & MySQL/Redis 可达与初始化 & \texttt{medicine} 已按基线和迁移脚本初始化 \\
