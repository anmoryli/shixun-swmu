...
| 文件 | 内容 |
|---|---|
| `慧医数字医疗应用系统_完整实习设计报告.pdf` | 57 页论文格式实习设计报告；含需求分析、概要设计、详细设计、测试结果、部署与持续集成、总结、参考文献及附录；嵌入图片目录全部 47 张 PNG |
| `慧医数字医疗应用系统_完整实习设计报告.tex` | 完整 LaTeX 源代码；使用 `ctexrep`、宋体正文、黑体黑色标题、三线表和自动目录，可在交付目录直接编译 |

本机 Tectonic 0.16.9 引擎及离线缓存保留在 `E:\Programs\Tectonic\0.16.9`，未安装到 C 盘。编译时设置 `TECTONIC_CACHE_DIR=E:\Programs\Tectonic\0.16.9\TectonicProject\Tectonic` 后调用 `tectonic.exe` 即可。
